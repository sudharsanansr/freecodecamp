(function() {
    ...
    if (new ITSRTCommonUtils().getRequestParams('LM_CMDB_Integration', 'proposedJSONFormat').toString().toLowerCase() === 'new') {
        while (importGr.next()) {
			...
			if((importGr.getValue('u_cpu_impact') != null) || (importGr.getValue('u_cpu_urgency') != null)){
            ...
            if((importGr.getValue('u_memory_impact') != null) || (importGr.getValue('u_memory_urgency') != null)){
				if(typeof jsonObjNew.threshold_settings == 'undefined'){
					jsonObjNew.threshold_settings = {};
				}
            ...
            gs.error('Genereated payload for CI : '+importGr.getValue('u_caption')+' : \n' + JSON.stringify(jsonObjNew));
        }
    }
	else{
		while (importGr.next()) {
